<template>
    <div class="flex flex-col">
            ...
            <div class="flex flex-col w-full h-auto mb-4 px-4 pt-3">
                ...
                <div class="flex w-full relative h-14">
                    ...
                    <button @click="handleUpdateInfo"
                        class="btn btn-primary rounded-xl btn-base flex w-32 h-6 border absolute top-0 right-0 justify-center items-center text-base">
                        编辑信息
                    </button>
                </div>
                ...
                <div class="flex flex-col w-full h-auto">
                    <div class="flex flex-1 flex-col mt-1 mb-3">
                        <span class=" tracking-wider flex text-xl font-bold">{{ userInfo?.username }}</span>
                        <span class=" tracking-wide flex text-sm font-light">{{ userInfo?.email }}</span>
                    </div>
                    <div class="flex w-full h-5 mb-3 tracking-wider">
                    ...
                    <div class="flex w-full h-8">
                        <NuxtLink class="tracking-wider text-xs font-normal mr-2" to="/focus">关注 {{ followCount }}
                        </NuxtLink>
                        <NuxtLink class=" tracking-wider text-xs font-normal ml-2"
                            :to="{ name: 'fans-id', params: { id: userInfo?.id } }">
                            粉丝 {{ fansCount }}
                        </NuxtLink>
                    </div>
                </div>
        ...
        <!-- 数据 -->
        <div class="flex tracking-widest flex-row w-full h-auto border-b border-base-300">
...
const avatar = ref('https://www.keaitupian.cn/cjpic/frombd/2/253/2139581748/1307183353.jpg')
