<template>
  <div class="flex items-center flex-row w-24 md:w-64">
    <div class="md:w-64 w-24 bg-base-200 top-0 h-screen fixed flex flex-col px-4 border-r border-base-300">
      <!-- logo -->
      ...
      <NuxtLink class="btn btn-ghost justify-start text-xl font-normal items-center rounded-full py-4 h-auto"
        ...
        <span v-if="(item.path === ('/' + useCenter.currentPath.split('/')[1]))" class="px-3 hidden md:block font-bold">{{
          item.name }}</span>
        <span v-else class="px-3 hidden md:block">{{ item.name }}</span>
      </NuxtLink>

      <!-- 发帖 -->
      <NuxtLink
        class="btn btn-ghost justify-center text-2xl font-normal items-center rounded-full py-4 mt-3 h-auto bg-base-300"
        onclick="postDialog.showModal()">
        发帖
      </NuxtLink>
